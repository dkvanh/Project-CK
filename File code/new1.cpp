...
int printICS()
{
    ...
    while(1)
    {
        ...
        for (i = 0; i < count_; i++)
        {
            gotoxy(40, 6 + i);
            color(Set[i]);
            printf("%d. Khoang phan ly thu %d: (%.3lf, %.3lf)", i + 1, i + 1, phanly[i].a, phanly[i].b);
        }
...
//Nhom ham tim MIX MAX ham so tren doan
double minf(double f(double x), ICS local)
{
